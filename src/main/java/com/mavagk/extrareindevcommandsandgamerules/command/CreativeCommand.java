...
public class CreativeCommand extends ModCommand {
	public CreativeCommand() {
		super("creative", true, false);
	}
	...
	public void onExecute(String[] args, ICommandListener commandExecutor) throws IllegalCmdListenerOperation {
		...
		for (EntityPlayer player : players) {
			boolean wasSuccessful = player.changeGamemode(1);
			if (wasSuccessful) {
				...
				if (wasSuccessful)
					commandExecutor.sendNoticeToOps("Setting " + player.username + " to creative mode");
				else
					commandExecutor.log(player.username + " if already in creative mode");
			}
		}
		if (players.size() > 1) {
	...
	public void printHelpInformation(ICommandListener commandExecutor) {
		commandExecutor.log("command.extrareindevcommandsandgamerules.creative.info0");
		commandExecutor.log("command.extrareindevcommandsandgamerules.creative.info1");
	}

	@Override
	public String commandSyntax() {
		return ChatColors.YELLOW + "/creative [players]";
	}
}
